...
void CFBXLoader::LoadMeshDataFromNode(FbxNode * _pNode)
{
	...
	if (pAttr && FbxNodeAttribute::eMesh == pAttr->GetAttributeType())
	{
		...
		if (NULL != pMesh)
		{
			string strInitialName = _pNode->GetInitialName();
			wstring strName = wstring(strInitialName.begin(), strInitialName.end());
			LoadMesh(pMesh, strName);
		}
...
void CFBXLoader::LoadMesh(FbxMesh * _pFbxMesh, wstring& strName)
{
...
{
	// 05/30 M - FBX에 저장된 파일이 아닌, 'Texture에 있는 같은 이름의 파일 이미지 불러오기'로 수정함.
	...
	for (UINT i = 0; i < m_vecContainer.size(); ++i)
	{
		for (UINT j = 0; j < m_vecContainer[i].vecMtrl.size(); ++j)
		{
			...
			strPath = strKey;
			strExt = CPathMgr::GetExt(m_vecContainer[i].vecMtrl[j].strDiff.c_str());	// 파일의 경로와 확장자까지
			strPath.append(strExt);
			std::vector<CBinaryFile> vecFiles = newDir.DirToAllFile(strPath.c_str());// 파일이름으로 Texture에 있는 파일 찾아오기	

			// 못찾으면 fbm 폴더에 있다는 뜻
			if (true == vecFiles.empty())
			{
			...
			{
				strPath = vecFiles[0].FilePath();										// 파일의 경로	
				CResMgr::GetInst()->Load<CTexture>(strKey, strPath, true);			// 파일 이름으로 텍스처 불러오기
			}


			// 2. 노말 불러오기		
			strKey = CPathMgr::GetFileName(m_vecContainer[i].vecMtrl[j].strNormal.c_str());
			strPath = strKey;
			strExt = CPathMgr::GetExt(m_vecContainer[i].vecMtrl[j].strNormal.c_str());	// 파일의 경로와 확장자까지
			strPath.append(strExt);
			vecFiles.clear();
			vecFiles = newDir.DirToAllFile(strPath.c_str());// 파일이름으로 Texture에 있는 파일 찾아오기	
			...
			// 3. 스펙큘러 불러오기
			strKey = CPathMgr::GetFileName(m_vecContainer[i].vecMtrl[j].strSpec.c_str());
			strPath = strKey;
			strExt = CPathMgr::GetExt(m_vecContainer[i].vecMtrl[j].strSpec.c_str());	// 파일의 경로와 확장자까지
			strPath.append(strExt);
			vecFiles.clear();
			vecFiles = newDir.DirToAllFile(strPath.c_str());// 파일이름으로 Texture에 있는 파일 찾아오기	
...
void CFBXLoader::CreateMaterial()
{
	...
	for (UINT i = 0; i < m_vecContainer.size(); ++i)
	{
		for (UINT j = 0; j < m_vecContainer[i].vecMtrl.size(); ++j)
		{
			...
			if (NULL != pTex)
			{
				pMaterial->SetData(SHADER_PARAM::TEX_0, &pTex);
			}
			else
			{
				assert(false);// 텍스처 key를 찾을 수 없음
			}
			...
			if (NULL != pTex)
			{
				pMaterial->SetData(SHADER_PARAM::TEX_1, &pTex);
			}
			else
			{
				assert(false);// 텍스처 key를 찾을 수 없음
			...
			if (NULL != pTex)
			{
				pMaterial->SetData(SHADER_PARAM::TEX_2, &pTex);
			}
			else
			{
				assert(false);// 텍스처 key를 찾을 수 없음
			...
			pMaterial->SetMaterialCoefficient(
				m_vecContainer[i].vecMtrl[j].tMtrl.vMtrlDiff
				, m_vecContainer[i].vecMtrl[j].tMtrl.vMtrlEmiv
				, m_vecContainer[i].vecMtrl[j].tMtrl.vMtrlSpec);

			CResMgr::GetInst()->AddRes<CMaterial>(pMaterial->GetName(), pMaterial);

			// 06/05 FBX로드 시, mtrl 확장자로 저장
			pMaterial->Save(pMaterial->GetPath());
		}
	}
}
...
void CFBXLoader::LoadAnimationClip()
{
	...
	for (int i = 0; i < iAnimCount; ++i)
	{
		...
		// rootNode->LclTranslationpro
		FbxAnimCurve* lCurve = rootNode->LclTranslation.GetCurve(pAnimLayer, FBXSDK_CURVENODE_COMPONENT_X);

		FbxProperty prop = pAnimLayer->GetSrcProperty();

		FbxAnimCurveNode* lCurveNode = prop.GetCurveNode(pAnimLayer);
