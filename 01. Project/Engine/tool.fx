...
float4 PS_Grid(VTX_OUT _in) : SV_Target
{
	...
	float fThickness = g_float_1 * (fDist / abs(g_vec4_0.y)) * 1.0f;
	...
	if (fX < fThickness / 2.f || fZ < fThickness / 2.f)
	{
		...
		return float4(0.5f, 0.f, 0.f, fRatio * fDepthRatio);
	}
	...
	if (fX < fThickness / 2.f || fZ < fThickness / 2.f)
	{
		...
		return float4(0.5f, 0.f, 0.f, fRatio * g_float_0 * fDepthRatio);
	}
