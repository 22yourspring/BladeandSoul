...
void CLayer::DeleteObj(CGameObject * _pObj)
{
	vector<CGameObject*>::iterator iter = m_vecLayerObj.begin();
	for (; iter != m_vecLayerObj.end(); ++iter)
	{
		if (*iter == _pObj)
		{
			m_vecLayerObj.erase(iter);
			return;
		}
	}
}
...
void CLayer::pause_update()
{
	for (UINT i = 0; i < m_vecParentObj.size(); ++i)
	{
		if (m_vecParentObj[i]->IsActive())
			m_vecParentObj[i]->pause_update();
	}
}

void CLayer::EngineUpdate()
{
	for (UINT i = 0; i < m_vecParentObj.size(); ++i)
	{
		m_vecParentObj[i]->EngineUpdate();
	}
}
